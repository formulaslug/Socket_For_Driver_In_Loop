...
class Tire:
    ...
    def __init__(self, normalForce, slipRatio, slipAngle, velocityX, pressure, temperature, camber, mechanicalParams, magicParams):
        ...
        self.normalForce = self.magic["loadA"] * (normalForce) ** 2 + self.magic["loadB"] * (normalForce) + self.magic["loadC"]
        ...
        self.camber = torch.tensor(0, dtype=torch.float32) #camber # Radians
    ...
    def calculateLongCompositeLongFrictionScalingFactor(self):
        return self.mechanical["friction-coeff-long"] / (1 + self.mechanical["friction-coeff-long"] * ((self.velocityX ) / torch.sqrt(torch.tensor(9.81 * self.mechanical["unloaded-radius"], dtype=torch.float32))))
